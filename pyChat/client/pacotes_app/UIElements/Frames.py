...
class addFrame_ui(Toplevel):
    def __init__(self, controller):
        ...
        self.add_button =  ttk.Button(self, text = 'Adicionar!', command= lambda: self.addFriend())
    ...
    def addFriend(self):
        friendEmail = self.entr_email.get()
        self.controller.addFriend(friendEmail)

    def addFeedback(self, feedback):
        pass

class addFeedback_ui(Toplevel):
    def __init__(self, controller):
        super().__init__(padx= 20,pady=20)
        self.controller = controller
        self.titulo = Label(self, text = 'Awaiting FeedBack. . .')
        self.titulo.grid(row = 0, column = 0, sticky = N+E+W)

        self.cancel_button =  ttk.Button(self, text = 'Ok!', command = self.destroy)
        self.cancel_button.grid(row = 4, column=0, sticky = E+W, padx = 15, pady = 5)

    def addSucces(self):
        self.titulo.config(text='Usuário adicionado com sucesso!')

    def notFoundUser(self):
        self.titulo.config(text='Usuário não encontrado!')
